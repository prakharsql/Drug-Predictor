...
import joblib
import pandas as pd
from .utils import SEX_MAP, BP_MAP, CHOL_MAP
...
def predict_single(model, age, sex, bp, chol, na_to_k) -> str:
    """
    Predict a single sample; returns predicted label (string).
    """
    if model is None:
    ...
    # If model returns encoded labels you may need to inverse-transform here.
    return str(pred[0])
